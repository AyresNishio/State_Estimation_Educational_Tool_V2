...
from SisPotFunctions import PSSE
...
#---Estimação de Estado
@app.callback([Output("output_state", "children"), Output("output_med", "children"),Output("graph","figure"),Output("criticalities", "data"), Output("criticalities", "columns"),Output('confirm', 'displayed')], [Input('input-se-top', 'children'), Input('input-se-med', 'children'), Input('exe-EE', 'n_clicks'), Input('select-ee', 'value'), Input("input_tol", "value"), Input("input_iteracoes", "value")], State('page-language', 'lang'))
#Saídas->>> y_bus_matrix, criticality_data, J_list, J_critical, State_dataframe, data_SE
def update_se(topology, meansured,n_clicks, metodo, tolerancia, total_max_iteracoes, language):
    if len(topology) > 2 and len(meansured) > 2:
        ...
        dict_names = {'De': 'De','Para':'Para','R':'R','X':'X','B':'B','Tap':'Tap'}
        ...
        if len(dash.callback_context.triggered):
            if dash.callback_context.triggered[0]['prop_id'] == 'exe-EE.n_clicks':
                ...
                criticality_data, J_list,J_critical, State_dataframe, data_SE,is_observable,_ = PSSE.state_estimation( line,med,tolerancia,metodo,total_max_iteracoes)
...
##Download Steps
@app.callback(Output('downloadSE-steps', "data"), [Input('input-se-top', 'children'), Input('input-se-med', 'children'), Input('exe-EE-step', 'n_clicks'),Input('select-ee', 'value'), Input("input_tol", "value"), Input("input_iteracoes", "value")], State('page-language', 'lang'))
#Saídas->>> y_bus_matrix, criticality_data, J_list, J_critical, State_dataframe, data_SE
def update_se_steps(topology, meansured,n_clicks, metodo, tolerancia, total_max_iteracoes,language):
    if len(topology) > 2 and len(meansured) > 2:
        data_1, data_2 = json.loads(topology), json.loads(meansured)
        line = pd.read_json(data_1['data'], orient='split')
        med = pd.read_json(data_2['data'], orient='split')
        ...
        if len(dash.callback_context.triggered):
            if dash.callback_context.triggered[0]['prop_id'] == 'exe-EE-step.n_clicks':
                ...
                if is_observable:
                    return dict(content =output_text, filename = 'Resultado Estimação.txt')
                    
                else:
                    #POP-UP NAO OBSERVAVEL
                    return dict(content = 'Rede Não Observável!', filename = 'teste.txt')
            else:
                return ''
